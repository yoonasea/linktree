...
// ── data ──────────────────────────────────────────────────────────────────────
const PROJECTS = [
  { id: 1, name: "Project Alpha",   from: "SFO", to: "NYC", date: "26 JUN 2026", seat: "12A", gate: "B4",  desc: "A fullstack web app with real-time features and a sleek dashboard. Built for scale and performance.", tags: ["React", "Node.js", "PostgreSQL"] },
  { id: 2, name: "Project Beta",    from: "LHR", to: "DXB", date: "14 MAY 2026", seat: "3F",  gate: "A22", desc: "An e-commerce platform with payment integration, inventory management and analytics.", tags: ["Next.js", "Stripe", "Prisma"] },
  { id: 3, name: "Project Gamma",   from: "NRT", to: "SIN", date: "02 APR 2026", seat: "7C",  gate: "C11", desc: "A mobile-first social app with live chat, push notifications and a recommendation engine.", tags: ["React Native", "Firebase", "Python"] },
  { id: 4, name: "Project Delta",   from: "CDG", to: "LAX", date: "18 MAR 2026", seat: "21D", gate: "D3",  desc: "An AI-powered productivity tool that summarises documents and drafts email replies.", tags: ["Python", "OpenAI", "FastAPI"] },
  { id: 5, name: "Project Epsilon", from: "SYD", to: "MEL", date: "07 FEB 2026", seat: "5B",  gate: "F9",  desc: "A DevOps dashboard for monitoring cloud infrastructure, costs and deployment pipelines.", tags: ["React", "AWS", "Docker"] },
];

const SKILL_GROUPS = [
  { label: "Frontend", icon: "\uD83D\uDDA5\uFE0F", color: SKY,  skills: ["React", "Next.js", "TypeScript", "Tailwind CSS", "Vue.js"] },
  { label: "Backend",  icon: "\u2699\uFE0F", color: MINT, skills: ["Node.js", "Express", "Python", "FastAPI", "PostgreSQL"] },
  { label: "DevOps",   icon: "\uD83D\uDE80", color: SKY,  skills: ["Docker", "AWS", "CI/CD", "Nginx", "Linux"] },
  { label: "Tools",    icon: "\uD83D\uDEE0\uFE0F", color: MINT, skills: ["Git", "Figma", "Redis", "GraphQL", "Prisma"] },
];
...
function ImagePlaceholder({ label }) {
  return (
    <div style={{
      flex: 1, minHeight: 96,
      background: "#f7f7f7", border: "1.5px dashed #ccc", borderRadius: 6,
      display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 5,
    }}>
      <span style={{ fontSize: 20, opacity: 0.35 }}>\uD83D\uDDBC\uFE0F</span>
      <span style={{ fontSize: 10, color: "#bbb", fontWeight: 700, letterSpacing: 1, textTransform: "uppercase" }}>{label}</span>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "14px 18px 12px", borderBottom: "1px solid #efefef", display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 10, color: "#bbb", letterSpacing: 2, fontWeight: 700, textTransform: "uppercase", marginBottom: 3 }}>
            Flight {String(index + 1).padStart(3, "0")}
          </div>
      ...
      <div style={{ display: "flex", gap: 8, padding: "12px 16px 0" }}>
        <ImagePlaceholder label="Screenshot 1" />
        <ImagePlaceholder label="Screenshot 2" />
      </div>
  ...
  return (
    <div ref={containerRef} style={{ height: "100vh", overflowY: "auto", fontFamily: "'Segoe UI', system-ui, sans-serif", background: CREAM, color: DARK, scrollBehavior: "smooth" }}>
      ...
      <section style={{ position: "relative", height: "100vh", marginTop: -53, overflow: "hidden", display: "flex", alignItems: "center" }}>
        ...
        <div style={{ position: "relative", zIndex: 2, padding: mobile ? "0 6vw" : "0 8vw", maxWidth: 680 }}>
          ...
          <h1 style={{ fontSize: mobile ? "2.1rem" : "clamp(2.4rem,5.5vw,4rem)", fontWeight: 900, lineHeight: 1.1, color: DARK, margin: "0 0 16px", textShadow: "0 2px 16px rgba(255,255,255,0.9)" }}>
            ...
            <span style={{ color: SKY }}>Building</span> things<br />
            that <span style={{ color: MINT }}>take off.</span>
          </h1>
          <p style={{ fontSize: 14, color: "#445", lineHeight: 1.75, margin: "0 0 26px", maxWidth: 400, background: "rgba(255,255,255,0.65)", padding: "10px 14px", borderRadius: 10 }}>
            Hi, I'm <strong>[Your Name]</strong> — a fullstack developer crafting fast, beautiful web experiences from pixel to deployment.
          </p>
      ...
      <section style={{ background: "#0f0f0f", padding: mobile ? "48px 20px" : "64px 40px", textAlign: "center" }}>
        ...
        <SolariBoard text={mobile ? "YOUR NAME" : "YOUR NAME HERE"} label="Passenger" size={mobile ? "sm" : "lg"} />
        <div style={{ marginTop: 20 }}>
          <SolariBoard text="FULLSTACK DEV" label="Role" size={mobile ? "sm" : "lg"} />
        </div>
        <div style={{ display: "flex", justifyContent: "center", gap: mobile ? 18 : 36, marginTop: 36, flexWrap: "wrap" }}>
          {[["STATUS", "BOARDING"], ["GATE", "WEB 3.0"], ["SEAT", "FRONTEND"]].map(([k, v]) => (
      ...
      <section id="skills" style={{ padding: mobile ? "56px 5vw" : "80px 8vw" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr 1fr" : "repeat(auto-fit,minmax(220px,1fr))", gap: 14 }}>
          {SKILL_GROUPS.map(g => (
            <div key={g.label} style={{ background: "#fff", border: "1.5px solid #e0ede8", borderRadius: 14, padding: mobile ? "16px 14px" : "22px 20px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                <div style={{ width: 32, height: 32, borderRadius: 8, background: g.color === SKY ? "rgba(79,163,209,0.12)" : "rgba(82,201,160,0.12)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>{g.icon}</div>
                <div style={{ fontWeight: 800, fontSize: 13, color: DARK }}>{g.label}</div>
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                {g.skills.map(s => (
                  <span key={s} style={{
                    background: g.color === SKY ? "rgba(79,163,209,0.09)" : "rgba(82,201,160,0.09)",
                    color: g.color,
                    border: `1px solid ${g.color === SKY ? "rgba(79,163,209,0.28)" : "rgba(82,201,160,0.28)"}`,
      ...
      <footer id="contact" style={{ background: DARK, color: "#fff", padding: mobile ? "56px 6vw 36px" : "72px 8vw 40px" }}>
        <div style={{ maxWidth: 560, margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", flexDirection: mobile ? "column" : "row", gap: 10, justifyContent: "center", alignItems: "center", marginBottom: 44 }}>
            <a href="mailto:you@email.com" style={{ background: SKY, color: "#fff", padding: "12px 26px", borderRadius: 10, fontWeight: 700, fontSize: 13, textDecoration: "none", width: mobile ? "100%" : "auto", boxSizing: "border-box", textAlign: "center" }}>
              ✉️ Send an email
            </a>
            <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" style={{ background: "rgba(255,255,255,0.07)", color: "#fff", padding: "12px 26px", borderRadius: 10, fontWeight: 700, fontSize: 13, textDecoration: "none", border: "1.5px solid rgba(255,255,255,0.14)", width: mobile ? "100%" : "auto", boxSizing: "border-box", textAlign: "center" }}>
              LinkedIn →
            </a>
            <a href="https://github.com" target="_blank" rel="noopener noreferrer" style={{ background: "rgba(255,255,255,0.07)", color: "#fff", padding: "12px 26px", borderRadius: 10, fontWeight: 700, fontSize: 13, textDecoration: "none", border: "1.5px solid rgba(255,255,255,0.14)", width: mobile ? "100%" : "auto", boxSizing: "border-box", textAlign: "center" }}>
              GitHub →
            </a>
          </div>
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.07)", paddingTop: 22, display: "flex", flexDirection: mobile ? "column" : "row", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontSize: 11, color: "rgba(255,255,255,0.25)" }}>© 2026 [Your Name]</span>
          </div>
        </div>
      </footer>
