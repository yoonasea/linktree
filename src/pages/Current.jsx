// Current.jsx — original portfolio design (Tailwind + framer-motion)
...
function Reveal({ children, dir = "up" }) {
  ...
  return <motion.div ref={ref} style={{ opacity, x, y }}>{children}</motion.div>
}
      ...
      {images.length > 0 ? (
        <div className={`relative h-48 bg-gradient-to-br ${s.img} overflow-hidden`}>
          <div className="relative w-full h-full overflow-hidden">
            <motion.div animate={{ x: `-${activeIdx * 100}%` }} transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }} className="flex h-full">
              {images.map((src, i) => (
